-----------------------
-- this is exactly the same format as the SYN one
-- but the division is that the whole set is for test image .
-- reserve one for train and one for validation, to make it compatible to existing work.
...
GenVecFromTo = utils.GenVecFromTo   -- take out
...
function Dataset:__init()   -- opt.idxRef.[train, valid, test],
    ...
    -- old version each time give one GenVec..
    --self.idxsTestSpec=                       -- specific index images, comment not needed one
    ---- ac2d P7
    ----GenVecFromTo(1,10)                         -- Naveen
    ----        --:cat(GenVecFromTo(11,23))       -- Shuangjun
    ----        :cat(GenVecFromTo(24,45))       -- Sarah wo coat
    ----        --:cat(GenVecFromTo(46, 66))      -- Sarah w coat
    ----        --:cat(GenVecFromTo(67, 83))      -- William
    --
    ---- ac2d sjl
    --GenVecFromTo(1,10)                         -- Naveen
    --        --:cat(GenVecFromTo(11,23))       -- Shuangjun
    ...
    if opt.testLs[1]:match('_sjl_') then
        print('sjl ac2d datasets')
        self.idxsTestSpec =
        GenVecFromTo(1,15)              -- gray down
        --:cat(GenVecFromTo(11,23))
                :cat(GenVecFromTo(31,45))   -- grid dressing shirt
        --:cat(GenVecFromTo(46, 66))
        --:cat(GenVecFromTo(67, 83))
    else
        print('ac2d p7 version dataset')
        self.idxsTestSpec =
            GenVecFromTo(1,10)                         -- Naveen
            --:cat(GenVecFromTo(11,23))       -- Shuangjun
            :cat(GenVecFromTo(24,45))       -- Sarah wo coat
            --:cat(GenVecFromTo(46, 66))      -- Sarah w coat
            --:cat(GenVecFromTo(67, 83))      -- Willia
    end


    print('generating AC2d dataset')
    ...
    local joints_gt=torch.FloatTensor()
    ...
    for i,v in ipairs(opt.testLs) do
        ...
        -- here is we set it to totally test set
        ...
        if opt.ifTsSpec then        -- just for one single version, not for multiple ac2d test here
            nTestTmp = self.idxsTestSpec:nElement()
            idxsTest = self.idxsTestSpec
        end
         --print('idxTest is', idxsTest)
    ...
    -- set opt.idxRef  train, test , vali, add on the opt variables
...
function Dataset:size(set)
    return self.nsamples[set]   -- can be zero
end
