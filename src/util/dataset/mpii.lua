...
function Dataset:__init()
    self.nJoints = 16
    self.accIdxs = {1,2,3,4,5,6,11,12,15,16}    -- 16 joints?  only eval 10?
    self.flipRef = {{1,6},   {2,5},   {3,4},
                    {11,16}, {12,15}, {13,14}}
    -- Pairs of joints for drawing skeleton, what order
    ...
    a:close()
    annot.index:add(1)  -- tensor type add 1 to start from 1, should be tensor
    annot.person:add(1)
    annot.part:add(1)
    ...
    if not opt.idxRef then
        ...
        if not opt.randomValid then
            ...
            -- Set up random training/validation split perm, take first half and last half
